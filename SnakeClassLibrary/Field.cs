...
namespace SnakeClassLibrary
{
    public sealed class Field 
    {
        ...
        public Game.GameEvent Update(Snake.Direction direction)
        {
            ...
            if (Snake.Blocks.Any(block => block.Coords.Equals(Snake.Head.Coords) && Snake.Blocks.IndexOf(block) != 0))
            {
            ...
            if (Snake.Head.Coords.Equals(food.Coords))
            {
                ...
                for (int i = 0; i < Snake.Blocks.Count; ++i)
                {
                    ...
                    {
                        Snake.Blocks[i] = 
                            new SnakeBlock(new Coordinates(this.Height - 1, Snake.Blocks[i].Coords.Column));
                    }
                    else
                    if (Snake.Blocks[i].Coords.Row == Height)
                    {
                        Snake.Blocks[i] =
                            new SnakeBlock(new Coordinates(0, Snake.Blocks[i].Coords.Column));
                    }
                    else
                    if (Snake.Blocks[i].Coords.Column < 0)
                    {
                        Snake.Blocks[i] =
                            new SnakeBlock(new Coordinates(Snake.Blocks[i].Coords.Row, this.Width - 1));
                    }
                    else
                    if (Snake.Blocks[i].Coords.Column == Width)
                    {
                        Snake.Blocks[i] =
                            new SnakeBlock(new Coordinates(Snake.Blocks[i].Coords.Row, 0));
                    }
                }
            }
            
            //remove non actual snake blocks
            foreach (var block in this.Blocks.Cast<Block>())
            {
                if (block is SnakeBlock && !Snake.Blocks.Any(sb => sb.Coords.Equals(block.Coords)))
                {
                    this.Blocks[block.Coords.Row, block.Coords.Column] = new FieldBlock(block.Coords);
                }
            }
